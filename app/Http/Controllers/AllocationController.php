...
class AllocationController extends Controller
{
    ...
    public function index(Request $request, FinanceService $finance, AllocationActualService $actuals, BudgetRuleService $budgetRules): Response
    {
        ...
        return Inertia::render('allocations', [
            ...
            'assets' => Asset::with(['buckets.goal', 'assetType'])->orderBy('name')->get()->map(fn (Asset $asset): array => ['id' => $asset->id, 'name' => $asset->name, 'type' => $asset->type, 'assetTypeLabel' => $asset->assetType?->label, 'assetClass' => $asset->assetType?->class, 'buckets' => $asset->buckets->map(fn ($bucket): array => ['id' => $bucket->id, 'name' => $bucket->name, 'goalName' => $bucket->goal?->name])->values()])->values(),
            'defaults' => [
